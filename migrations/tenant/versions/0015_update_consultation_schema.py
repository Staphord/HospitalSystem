...
def upgrade() -> None:
    ...
    if "consultation_status" not in cols:
    ...
    if "fee_schedules" not in tables:
        op.create_table(
            "fee_schedules",
            sa.Column("fee_id", sa.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
    ...
    op.execute("""
        INSERT INTO fee_schedules (fee_id, item_code, item_name, item_type, standard_price, insurance_price, is_active, effective_from)
        VALUES 
...
def downgrade() -> None:
    ...
    if "bill_items" in tables: op.drop_table("bill_items")
    if "bills" in tables: op.drop_table("bills")
    
    # Drop fee_schedules
    if "fee_schedules" in tables: op.drop_table("fee_schedules")

    # Drop prescriptions
    if "prescriptions" in tables: op.drop_table("prescriptions")
    ...
    # Remove columns from consultations if they exist
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    cols = [c["name"] for c in inspector.get_columns("consultations")]
    
    if "referral_notes" in cols: op.drop_column("consultations", "referral_notes")
    if "referral_type" in cols: op.drop_column("consultations", "referral_type")
    if "disposition" in cols: op.drop_column("consultations", "disposition")
    if "completed_at" in cols: op.drop_column("consultations", "completed_at")
    if "started_at" in cols: op.drop_column("consultations", "started_at")
    if "consultation_status" in cols: op.drop_column("consultations", "consultation_status")
