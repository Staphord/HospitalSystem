...
def _create_superuser(username: str, password: str, email: str, full_name: str = "") -> dict | str:
...
def _update_superuser(sub: str, username: str | None = None, full_name: str | None = None, password: str | None = None, email: str | None = None) -> bool | str:
    payload = {}
    if username is not None:
        payload["username"] = username
    if full_name is not None:
    ...
    if email is not None:
        payload["email"] = email
    r = _client().patch(f"/superadmin/users/{sub}", json=payload)
    if r.status_code == 200:
        return True
    detail = r.json().get("detail", r.text) if r.text else f"HTTP {r.status_code}"
    return detail
...
ALL_ROLES = ["hospital_admin", "hospital_user", "nurse", "clinician", "doctor", "patient"]
    ...
    if is_super:
            ...
            for u in users:
                ...
                if st.session_state.get(edit_key, False):
                    with st.container(border=True):
                        ...
                        if save:
                            kwargs = {"sub": sub, "full_name": new_full_name, "username": new_username, "email": new_email}
                            if new_password:
                ...
                if st.session_state.get(delete_key, False):
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("Yes, delete", key=f"sa_confirm_del_{sub}"):
                    ...
                    with col2:
                        if st.button("Cancel", key=f"sa_cancel_del_{sub}"):
                            st.session_state[delete_key] = False
                            st.rerun()
                st.divider()
        else:
        ...
        if create_clicked:
            if not all([new_username, new_email, new_password]):
                ...
                result = _create_superuser(new_username, new_password, new_email, new_full_name)
                if isinstance(result, dict):
...
def _show_dashboard():
    ...
    with st.sidebar:
        ...
        st.markdown(f"**Role:** {'Super Admin' if is_super else 'Hospital Admin'}")
        st.markdown("---")
        nav_options = ["Users", "Tenants", "Profile"] if is_super else ["Users", "Profile"]
