...
# Register UUID type compiler for SQLite compatibility
...
def fixture_db_session():
    ...
    # Use a truly isolated in-memory SQLite per test — StaticPool ensures
    # the same connection is reused within the test, and the DB is thrown
    # away afterwards, preventing UNIQUE constraint leakage between tests.
...
def mock_external_services():
    # The router imports keycloak functions BY NAME at module level, so we
    # must patch them in the router's namespace, not just the source module.
    with patch("app.api.v1.superadmin.router.setup_tenant_realm", new_callable=AsyncMock), \
         patch("app.api.v1.superadmin.router.verify_tenant_realm_exists", new_callable=AsyncMock, return_value=True), \
         patch("app.api.v1.superadmin.router.ensure_roles", new_callable=AsyncMock), \
         patch("app.api.v1.superadmin.router.create_keycloak_user", new_callable=AsyncMock, return_value="kc-dummy-sub"), \
         patch("app.api.v1.superadmin.router.create_local_user") as mock_create_local, \
        ...
        # Fernet encryption is not testable without a real key; mock at source module.
        with patch("app.services.tenant_service.encrypt_dsn", return_value="encrypted-dsn"):
            yield
...
def test_generate_invoice(client, db_session):
    ...
    tenant = Tenant(tenant_id="t1", hospital_name="Hosp 1", is_active=True, status="active",
                    subscription_plan="standard", db_connection_string="dummy",
                    created_by=uuid.UUID("de305d54-75b4-431b-adb2-eb6b9e546014"))
    db_session.add(tenant)

    # Add a super admin referenced by created_by FK
    from datetime import datetime, timezone
    admin = SuperAdmin(
        super_admin_id=uuid.UUID("de305d54-75b4-431b-adb2-eb6b9e546014"),
        username="superadmin", email="superadmin@example.com",
        password_hash="dummy", full_name="Super Admin",
        role="super_admin", mfa_secret="dummy",
    )
    db_session.add(admin)

    # Add an active subscription so the invoice endpoint can resolve subscription_id
    ...
    # Payload now matches InvoiceCreate schema
    payload = {
    ...
    data = response.json()
    assert str(data["amount"]) == "1500.00"
...
def test_record_payment(client, db_session):
    ...
    payload = {
        "invoice_id": str(invoice.invoice_id),  # UUID must be serialised to string for JSON
        "amount": 1500,
        "payment_method": "card",
        "reference_number": "REF123"
    }
    response = client.post("/api/v1/superadmin/tenants/t1/payments", json=payload)
    assert response.status_code == 201  # endpoint uses status_code=201
    data = response.json()
    # amount is stored as Numeric/Decimal — serialised as a string in JSON
...
def test_announcements(client, db_session):
    # Create announcement — uses legacy field names (message/scope) that the
    # schema now accepts via resolved_body / resolved_audience properties.
    payload = {
        "title": "System Update",
    ...
    assert data["body"] == "Scheduled maintenance"
...
# NOTE: The /audit-log endpoint queries SuperAdminAuditLog, not GlobalAuditLog.
# We seed a SuperAdminAuditLog row so the endpoint returns it.
...
# (duplicate import block removed — all imports are at the top of the file)
...
def test_create_superadmin_user(client, db_session):
    ...
    # ensure_roles is imported by name in the router module, so we must patch
    # it in the router's namespace, not in keycloak_admin's namespace.
    with patch("app.api.v1.superadmin.router.ensure_roles", new_callable=AsyncMock), \
         patch("app.api.v1.superadmin.router.create_keycloak_user", new_callable=AsyncMock, return_value="kc-new-sub"), \
         patch("app.api.v1.superadmin.router.settings.smtp_user", "smtp_user"), \
         patch("app.api.v1.superadmin.router.settings.smtp_password", "smtp_pass"), \
         patch("app.services.keycloak_admin.create_local_user"), \
         patch("app.services.superadmin_auth.create_superadmin") as mock_create_admin, \
         patch("aiosmtplib.send", new_callable=AsyncMock) as mock_send_email:

        # Build a minimal mock SuperAdmin object returned by create_superadmin
        mock_admin = MagicMock()
        mock_admin.super_admin_id = "de305d54-75b4-431b-adb2-eb6b9e546014"
        mock_admin.username = "new_superadmin_test"
        mock_admin.email = "new_test_admin@example.com"
        mock_admin.full_name = "New Test Admin"
        mock_admin.role = "super_admin"
        mock_admin.is_active = True
        mock_admin.last_login_at = None
        mock_admin.mfa_secret = "MOCKSECRET"
        mock_admin.backup_codes = None
        mock_admin.plaintext_backup_codes = ["code1", "code2"]
        from datetime import datetime, timezone
