...
async def _get_async_session_factory(tenant_id: str) -> async_sessionmaker:
    ...
    engine = create_async_engine(
        ...
        max_overflow=10,
        pool_recycle=3600,
        # SQL echo stays off in every environment, unlike the other services.
        # Echo logs each statement together with its bound parameters, which
        # here means visit and patient identifiers, and in the allergy query the
        # patient's recorded allergy text. A clinical service must not put that
        # in an ordinary application log just because it is running in dev.
        echo=False,
    )
